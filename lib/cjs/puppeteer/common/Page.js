...
class Page extends EventEmitter_js_1.EventEmitter {
    ...
    async screencast(options) {
        ...
        return new Promise(async (resolve) => {
          await this._client.send('Page.startScreencast', {...defaultOptions, options});
          this._client.on('Page.screencastFrame', ({ data, sessionId }) => {
            observer.send(this, "screen-frame", data);
            // Do someFthing with `data` and ack the frame if we can
            this._client.send('Page.screencastFrameAck', { sessionId }).catch(() => { });
          });
          resolve(observer);
        })
    }
